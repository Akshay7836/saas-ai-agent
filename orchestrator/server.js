...
// 🚀 API 1: Scan Repository (Same as before)
app.post('/scan', async (req, res) => {
    const { repo } = req.body;
    if (!repo || !repo.includes('/')) {
        return res.status(400).json({ explanation: "Invalid format. Use 'owner/repo'." });
...
// 🛠️ API 2: Updated PR-based Apply Fix
app.post('/apply-fix', async (req, res) => {
    try {
        ...
        // 1. Get Fix from Python Engine
        const aiRes = await axios.post(`${PYTHON_URL}/apply-fix`, req.body);
        const fixedCode = aiRes.data.fixed_code; // Expecting Python to return the code
        ...
        // 5. Get File SHA from Main (to update)
        ...
        const pr = await octokit.pulls.create({
            ...
            head: newBranch,
            base: 'main',
            body: "This PR was generated by Gemini AI Agent. It includes logical optimizations and error handling improvements."
        });
        ...
        res.status(500).json({ status: "error", message: "Failed to create PR." });
    }
});
